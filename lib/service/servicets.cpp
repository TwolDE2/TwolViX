...
{
	eDebug("[servicets]ServiceTS construct!");
...
{
	eDebug("[servicets]ServiceTS destruct!");
	stop();
}
...
int eServiceTS::openHttpConnection(std::string url)
{
	...
	eDebug("[servicets]connecting to %s", url.c_str());
	...
	//eDebug("[servicets]RECV(%d): %s", rc, linebuf);
	if (rc <= 0)
	{
		close(fd);
		free(linebuf);
	...
	if (rc != 3 || statuscode != 200) {
		eDebug("[servicets]wrong response: \"200 OK\" expected.");
		free(linebuf);
		close(fd);
		return -1;
	}
	eDebug("[servicets]proto=%s, code=%d, msg=%s", proto, statuscode, statusmsg);
	while (rc > 0)
	{
		rc = getline(&linebuf, &buflen, fd);
		//eDebug("[servicets]RECV(%d): %s", rc, linebuf);
...
RESULT eServiceTS::start()
{
	...
	if (dvbChannel.getDemux(m_decodedemux, iDVBChannel::capDecode) != 0) {
		eDebug("[servicets]Cannot allocate decode-demux");
		return -1;
	}
	if (m_decodedemux->getMPEGDecoder(m_decoder) != 0) {
		eDebug("[servicets]Cannot allocate MPEGDecoder");
		return -1;
	}
	if (m_destfd == -1)
	{
		...
		if (m_destfd < 0)
		{
			eDebug("[servicets]openDVR failed");
			return -1;
		}
	}
...
{
	eDebug("[servicets]eServiceTS::recv_event: %d", evt);
	switch (evt) {
...
RESULT eServiceTS::unpause()
{
	if (!m_streamthread->running())
	{
		...
		if (srcfd < 0) {
			eDebug("[servicets]Cannot open source stream: %s", m_filename.c_str());
			return 1;
		}
		...
		eDebug("[servicets]unpause but thread already running!");
	return 0;
}
...
void eStreamThread::thread() {
	...
	r = w = 0;
	hasStarted();
	eDebug("[servicets]eStreamThread started");
	while (!m_stop) {
		pthread_testcancel();
		FD_ZERO(&rfds);
		FD_ZERO(&wfds);
		...
		if (rc == 0) {
			eDebug("[servicets]eStreamThread::thread: timeout!");
			continue;
		}
		if (rc < 0) {
			eDebug("[servicets]eStreamThread::thread: error in select (%d)", errno);
			break;
		}
		if (FD_ISSET(m_srcfd, &rfds)) {
			rc = ::read(m_srcfd, buf+r, bufsize - r);
			if (rc < 0) {
				eDebug("[servicets]eStreamThread::thread: error in read (%d)", errno);
				...
				r += rc;
				if (r == bufsize) eDebug("[servicets]eStreamThread::thread: buffer full");
			}
		}
		if (FD_ISSET(m_destfd, &wfds) && (w < r) && ((r > bufsize/4) || eof)) {
			rc = ::write(m_destfd, buf+w, r-w);
			if (rc < 0) {
				eDebug("[servicets]eStreamThread::thread: error in write (%d)", errno);
				m_messagepump.send(evtWriteError);
				break;
			}
			w += rc;
			//eDebug("[servicets]eStreamThread::thread: buffer r=%d w=%d",r,w);
			if (w == r) {
	...
	eDebug("[servicets]eStreamThread end");
}

void eStreamThread::thread_finished() {
	if (m_srcfd >= 0)
		::close(m_srcfd);
	eDebug("[servicets]eStreamThread closed");
	m_running = false;
}
