...
{
	m_is_streamx = m_is_stream;	// sets to false as m_is_stream(ref.path.find("://") is not looking at fallback url at this point.
...
void eDVBServicePlay::setCutList(ePyObject list)
{
	...
	for (i=0; i<size; ++i)
	{
		...
		{
			eDebug("[servicedvb][eDVBServicePlay] cutlist entries need to be (pts, type)-tuples (%d %d)", PyLong_Check(ppts), PyInt_Check(ptype));
			continue;
		}
