	...
	switch (GST_MESSAGE_TYPE (msg))
	{
		...
		case GST_MESSAGE_BUFFERING:
			if (m_sourceinfo.is_streaming)
			{
				...
				eLog(5, "[eServiceMP3] Buffering %u percent done", m_bufferInfo.bufferPercent);
...
void eServiceMP3::pullSubtitle(GstBuffer *buffer)
{
	if (buffer && m_currentSubtitleStream >= 0 && m_currentSubtitleStream < (int)m_subtitleStreams.size())
	{
		...
		size_t len = map.size;
		eLog(5, "[eServiceMP3] gst_buffer_get_size %zu map.size %zu", gst_buffer_get_size(buffer), len);
		gint64 duration_ns = GST_BUFFER_DURATION(buffer);
		int subType = m_subtitleStreams[m_currentSubtitleStream].type;
		eLog(5, "[eServiceMP3] pullSubtitle type=%d size=%zu", subType, len);
		if ( subType )
		{
			if ( subType < stVOB )
			{
				...
				eLog(5, "[eServiceMP3] got new text subtitle @ buf_pos = %lld ns (in pts=%lld), dur=%lld: '%s' ", buf_pos, buf_pos/11111, duration_ns, line.c_str());
