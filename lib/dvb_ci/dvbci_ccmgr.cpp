...
int eDVBCICcSession::generate_dh_key()
{
	...
	eDebug("[dvbci_ccmgr][generate_dh_key][CI%d RCC]1 generate key...", m_slot->getSlotID());
	m_dh = DH_new();
	if (!m_dh)
	{
	...
	if (!useThis)
	{
		...
		{
			eDebug("[dvbci_ccmgr][generate_dh_key][CI%d RCC]2b DH_generate_key failed, error: %d ", m_slot->getSlotID(), ERR_error_string(err, NULL));				   
		}	
//		eDebug("[dvbci_ccmgr][generate_dh_key][CI%d RCC]2b DH_generate_key failed error: %d ", m_slot->getSlotID(), ERR_error_string(ERR_get_error(), NULL));
		return -1;
	}
