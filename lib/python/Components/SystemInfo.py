...
class BoxInformation:
	...
	def setItem(self, item, value, immutable=False):
		if item in self.immutableList:
			print("[BoxInfo] Error: Item '{item}' is immutable and can not be %s!" % ("changed" if item in self.boxInfo else "added"))
			return False
...
SystemInfo["Passthrough_fix_needed"] = SystemInfo["boxtype"] in ("vuultimo4k", "vuduo4k", "vuduo4kse")
...
SystemInfo["CanNotDoSimultaneousTranscodeAndPIP"] = SystemInfo["boxtype"] in ("vusolo4k", "gbquad4k", "gbue4k", "gbquad4kpro")
