...
class ServiceInfoET8500blue(Converter, object):
	HAS_TELETEXT = 0
	IS_MULTICHANNEL = 1
	IS_CRYPTED = 2
	IS_WIDESCREEN = 3
	...
	def __init__(self, type):
		...
		self.type, self.interesting_events = {
				...
				"IsMPIII": (self.IS_MPIII, (iPlayableService.evUpdatedInfo,)),
			}[type]

	def getServiceInfoFTCString(self, info, what, convert = lambda x: "%d" % x):
		v = info.getInfo(what)
		if v == -1:
			return "N/A"
		if v == -2:
		...
		if self.type == self.HAS_TELETEXT:
			...
			if audio:
				...
				while idx < n:
					i = audio.getTrackInfo(idx)
					description = i.getDescription()
					if "AC3" in description or "AC-3" in description or "DTS" in description  or "MP3" in description or "MPEG" in description or "WMA" in description:
						return True
					idx += 1
			return False
