...
class CreateQuadPipChannelEntry(ChannelSelectionBase):
	skin_default_1080p = """
		<screen name="CreateQuadPipChannelEntry" position="center,center" size="1500,850" flags="wfNoBorder">
			...
			<widget name="textChannels" position="100,580" size="1250,34" font="Regular;32" transparent="1" />
...
class QuadPipScreen(Screen, FocusShowHide, HelpableScreen):
	...
	def __onClose(self):
		...
		if self.newService and (not self.oldService or self.newService != self.oldService):
			self.oldService = self.newService
	...
	def keyOk(self):
		if self.isShown():
			...
			if channel:
				...
				if chInfo:
					(sname, sref) = chInfo
					self.newService = eServiceReference(sref)
					self.close()
		else:
			self.showFocus()
	...
	def ChannelSelectCB(self, force_exit=False):
		if force_exit:
			self.close()
			return
		if self.qpipChannelList.length() == 0:
	...
	def playChannel(self, channel):
		...
		for idx in list(range(1, 5)):
			...
			if chInfo is None:
				self.session.qPips.append(None)
	...
	def disableLcdLiveTV(self):
		try:
			self.oldLcdLiveTVEnable = config.plugins.LcdLiveTV.enable.value
	...
	def enableLcdLiveTV(self):
		if self.oldLcdLiveTVEnable:
			try:
				config.plugins.LcdLiveTV.enable.value = self.oldLcdLiveTVEnable
			except:
				pass
