...
class HD51Imager(Screen):
	...
	def __init__(self, session, args = 0):
		...
		if SystemInfo["HaveMultiBoot"]:
			...
			self["info-multi"] = Label(_("Select with yellow the OnlineFlash Image\n or select Recovery to create a USB Disk Image for clean Install."))
			self["info-usb"] = Label(_("USB or HDD = Back-up on USB or HDD?\nBackup takes between 4 -> 15 minutes depending on filesystem.\nFirst insert an USB before you select USB."))
			self["info-hdd"] = Label(_(" "))
		else:
			self["key_yellow"] = StaticText("")
			self["info-multi"] = Label(" ")
			self["info-usb"] = Label(_("USB = Back-up on USB?\nThis will take between 4 -> 15 minutes depending on filesystem.\nFirst insert an USB before you select USB."))
	...
	def doFullBackup(self, DIRECTORY):
		...
		if self.ROOTFSTYPE == "ubi":
			...
			self.message += _("because of the used filesystem the back-up\n")
			self.message += _("will take about 5-6 minutes for this system\n")
		else:
	...
	def imageInfo(self):
		...
		AboutText += _("Thanks to openATV Image Team") + "\n"
