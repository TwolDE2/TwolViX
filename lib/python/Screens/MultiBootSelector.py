...
#		canAddUsbMultiboot = SystemInfo["HasKexecMultiboot"] and not SystemInfo["HasKexecUSB"]
		...
		self["key_red"] = StaticText(_("Add Multiboot USB") if SystemInfo["HasKexecMultiboot"] else _("Cancel"))
		...
		self["actions"] = HelpableActionMap(self, ["OkCancelActions", "ColorActions", "DirectionActions", "KeyboardInputActions", "MenuActions"], {
			"red": (self.KexecMount if SystemInfo["HasKexecMultiboot"] else self.cancel, _("Add Vu+ USB slots") if SystemInfo["HasKexecMultiboot"] else _("Cancel the image selection and exit")),
	...
	#						print("[MultiBootSelector] key, not in line ", usblist[hddkey], "   ", xline)											 
			...
			self.session.openWithCallback(self.addSTARTUPs, MessageBox, _("Starting in slot %s, add 8 more Vu Multiboot USB slots ?" % hiKey), MessageBox.TYPE_YESNO, timeout=30)
