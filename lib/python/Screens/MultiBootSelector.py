...
class MultiBootSelector(Screen, HelpableScreen):
	...
	def getBootOptions(self, value=None):
		...
		if path.isdir("/tmp/startupmount") and path.ismount("/tmp/startupmount"):
			print "[MultiBootSelector] reboot0 already have startupmount"
			self.getImagesList()
		else:
			print "[MultiBootSelector] reboot0 make startupmount and load bootdevice"
	...
	def reboot(self):
		...
		if self.currentSelected[0][1] != "Queued":
			...
			if self.slot < 12:
				...
				print "[MultiBootSelector] reboot4 startupfile = %s" % startupfile
