			...
			if self.timerType == TIMERTYPE.WAKEUP:
				...
				if not Screens.Standby.inTryQuitMainloop:  # not a shutdown messagebox is open
					if Screens.Standby.inStandby:  # in standby
						...
						Notifications.AddNotificationWithUniqueIDCallback(self.sendTryToRebootNotification, "PT_StateChange", MessageBox, (_("A finished powertimer wants to reboot your ") + f"{SystemInfo['MachineBrand']} {SystemInfo['MachineName']}" + _(".\nDo that now?")), timeout=180)
				return True
