...
int eDVBDemux::openDemux(void)
{
	...
	if (tmp_fd == 0)
	{
		...
		/* eDebug("[eDVBDemux] opening null fd returned: %d", myFdKluge); */
	}
	if (tmp_fd != -1)
	{
		::close(tmp_fd);
...
void eDVBRecordFileThread::flush()
{
	...
	eDebug("[eDVBDemux][eDVBRecordFileThread] buffer usage histogram (%d buffers of %jd kB)", bufferCount, (intmax_t)m_buffersize>>10);
...
RESULT eDVBTSRecorder::start()
{
	...
	if (tmp_fd == 0)
	{
		...
		tmp_fd = -1;	
		myFdKluge = ::open("/dev/null", O_RDONLY | O_CLOEXEC);
		/* eDebug("[eDVBTSRecorder] opening null fd returned: %d", myFdKluge); */
	}
	if (tmp_fd != -1)
	{
		::close(tmp_fd);
