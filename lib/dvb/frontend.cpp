...
eDVBFrontend::eDVBFrontend(const char *devicenodename, int fe, int &ok, bool simulate, eDVBFrontend *simulate_fe)
	:m_simulate(simulate),console_fd(-1), m_enabled(false), m_fbc(false), m_simulate_fe(simulate_fe), m_type(-1), m_dvbid(fe), m_slotid(fe)
...
int eDVBFrontend::openFrontend()
{
	...
	if (!m_simulate)
	{
		...
		if (m_fd < 0)
		{
			...
			eDebugNoSimulate("[eDVBFrontend] opening console fd returned: %s", console_fd);
