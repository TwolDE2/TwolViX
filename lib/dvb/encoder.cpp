...
int eEncoder::allocateHDMIEncoder(const std::string &serviceref, int &buffersize)
{
	...
	eDebug("[eEncoder][allocateHDMIEncoder] framerate = %d bitrate=%d width=%d height=%d vcodec=%s acodec=%s", hdmi_encoding_framerate, hdmi_encoding_bitrate, hdmi_encoding_width, hdmi_encoding_height, hdmi_encoding_vcodec, hdmi_encoding_acodec);
